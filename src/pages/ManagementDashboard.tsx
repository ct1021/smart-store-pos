...
const ManagementDashboard: React.FC = () => {
    ...
    const parseVoiceInput = (text: string) => {
        if (voiceMode === 'sale') {
            // 模式: "可口可乐 3瓶 9块" or "可口可乐 三瓶 九块"
            const match = text.match(/([\u4e00-\u9fa5a-zA-Z0-9]+)\s*(\d+|[一二三四五六七八九十百]+)\s*瓶?\s*(\d+|[一二三四五六七八九十百]+)\s*块?/);

            if (match) {
                const productName = match[1];
                const quantity = parseInt(match[2]) || chineseToNumber(match[2]);
                const amount = parseFloat(match[3]) || chineseToNumber(match[3]);
                ...
                addOrder(newOrder);
                showToast(`记账成功: ${productName} x${quantity} = ¥${amount}`, 'success');
                setActiveModal(null);
            } else {
                showToast('未能识别语音内容，请重试', 'error');
            }
        } else if (voiceMode === 'restock') {
            // 模式: "进货 可口可乐 50瓶 120块"
            const match = text.match(/进货\s*([\u4e00-\u9fa5a-zA-Z0-9]+)\s*(\d+)\s*瓶?\s*(\d+)\s*块?/);
            ...
            if (match) {
                ...
                addExpense({
                    id: Date.now(),
                    name: `进货: ${productName} x${quantity}`,
                    amount: cost,
                    date: todayStr,
                    category: 'restock'
                });

                showToast(`进货记录成功: ${productName} x${quantity} = ¥${cost}`, 'success');
                setActiveModal(null);
            } else {
                showToast('未能识别进货信息，请重试', 'error');
            }
        }
    };
    ...
    const chineseToNumber = (chinese: string): number => {
        const mapping: { [key: string]: number } = { '零': 0, '一': 1, '二': 2, '三': 3, '四': 4, '五': 5, '六': 6, '七': 7, '八': 8, '九': 9, '十': 10, '百': 100 };
        if (chinese in mapping) return mapping[chinese];
            ...
            {activeModal === 'voice' && (
                <div className="fixed inset-0 z-50 flex flex-col bg-zinc-950/95 backdrop-blur-xl animate-in fade-in duration-200">
                        ...
                        <div className="text-center space-y-8 px-6">
                            {isListening ? (
                                <div className="space-y-4">
                                    <p className="text-primary font-bold text-xl animate-pulse">正在听...</p>
                                    {voiceTranscript && <p className="text-white text-sm">{voiceTranscript}</p>}
                                </div>
                            ) : (
                                <div className="space-y-4 opacity-50">
                                    <Mic size={40} className="text-zinc-500 mx-auto" />
                                    <p className="text-zinc-300 font-bold text-lg">按住说话</p>
                                    <p className="text-zinc-500 text-xs">
                                        {voiceMode === 'sale' ? '例如: "可口可乐 3瓶 9块"' : '例如: "进货 可口可乐 50瓶 120块"'}
                                    </p>
                                </div>
                            )}
                        </div>
                    ...
                    <div className="h-1/3 bg-zinc-900 rounded-t-[3rem] flex items-center justify-center pb-8 border-t border-zinc-800 shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
                        <button
                            className={`w-32 h-32 rounded-full flex items-center justify-center transition-all duration-200 ${isListening ? 'bg-primary scale-110 shadow-[0_0_50px_rgba(255,59,48,0.5)]' : 'bg-zinc-800 text-zinc-400'}`}
